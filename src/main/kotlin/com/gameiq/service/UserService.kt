...
class QuizService(
    ...
    fun generateQuiz(
        ...
        // Check rate limits for users without active subscriptions
    ...
    // Rate limiting for users without active subscriptions
    private fun checkQuizRateLimit(user: User) {
        if (user.subscriptionTier == SubscriptionTier.NONE) {
            throw IllegalStateException("Active subscription required for quiz features. Choose Basic ($9.99) or Premium ($14.99) to unlock unlimited quizzes.")
        }
        
        // Both BASIC and PREMIUM users get unlimited quiz access
        // Quiz generation doesn't use Claude API directly (it's part of the coaching system)
        // so no additional rate limiting needed here
    }
